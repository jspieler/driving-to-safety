function calculate() {
	...
	var numberOfFatalities = parseFloat(document.getElementById('fatalities').value);
	
	const rates = [numberOfFatalities, numberOfInjuries, numberOfCrashes];
	const labels = ['Fatalities', 'Injuries', 'Crashes'];
	
	var unit = document.getElementById('unit').value;
	var confidenceInterval = parseFloat(document.getElementById('confidence').value);
	...
	function formatNumber(num) {
		...
		if (Math.abs(num) >= 1e9) {
			// Divide the number by 1 billion and round to 1 decimal place
			return (num / 1e9).toFixed(1) + 'B';
		}
		// If the number is greater than or equal to a million
		...
		// Otherwise, return the original number
			...
			return num.toLocaleString(); // Fallback to default number formatting
		}
	}
	...
	const distanceCrashesBenchmark = Math.ceil(Math.log(1 - confidenceInterval) / Math.log(1 - numberOfCrashes));
	const distanceInjuriesBenchmark = Math.ceil(Math.log(1 - confidenceInterval) / Math.log(1 - numberOfInjuries));
	const distanceFatalitiesBenchmark = Math.ceil(Math.log(1 - confidenceInterval) / Math.log(1 - numberOfFatalities));
	...
	resultsBenchmark.innerHTML = `How many ${unit} would autonomous vehicles have to be driven without failure to demonstrate with ${(confidenceInterval * 100).toString()} % confidence that their failure rate is at most`;
	
	var results = '<ul>';
	results += `<li>${numberOfCrashes} crashes per ${unit}: ${formatNumber(distanceCrashesBenchmark)} ${unit}</li>`;
	results += `<li>${numberOfInjuries} injuries per ${unit}: ${formatNumber(distanceInjuriesBenchmark)} ${unit}</li>`;
	results += `<li>${numberOfFatalities} fatalities per ${unit}: ${formatNumber(distanceFatalitiesBenchmark)} ${unit}</li>`;
	results += '</ul>';
	resultsBenchmark.innerHTML += results;

	// Visualize result in a chart
	...
	const distanceCrashesPrecision = Math.ceil(Math.pow(zScore / precision, 2) / numberOfCrashes);
	const distanceInjuriesPrecision = Math.ceil(Math.pow(zScore / precision, 2) / numberOfInjuries);
	const distanceFatalitiesPrecision = Math.ceil(Math.pow(zScore / precision, 2) / numberOfFatalities);
	...
	resultsPrecision.innerHTML = `How many ${unit} would autonomous vehicles have to be driven to demonstrate with ${(confidenceInterval * 100).toString()} % confidence their failure rate is within ${(precision * 100).toString()} % of the true rate of`;
	
	var results = '<ul>';
	results += `<li>${numberOfCrashes} crashes per ${unit}: ${formatNumber(distanceCrashesPrecision)} ${unit}</li>`;
	results += `<li>${numberOfInjuries} injuries per ${unit}: ${formatNumber(distanceInjuriesPrecision)} ${unit}</li>`;
	results += `<li>${numberOfFatalities} fatalities per ${unit}: ${formatNumber(distanceFatalitiesPrecision)} ${unit}</li>`;
	results += '</ul>';
	...
	const distanceCrashesImprovement = Math.ceil(improvedCrashRate * Math.pow(zScoreFull / (numberOfCrashes - improvedCrashRate), 2));
	
	const improvedInjuryRate = (1 - improvement) * numberOfInjuries;
	const distanceInjuriesImprovement = Math.ceil(improvedInjuryRate * Math.pow(zScoreFull / (numberOfInjuries - improvedInjuryRate), 2));
	
	const improvedFatalityRate = (1 - improvement) * numberOfFatalities;
	const distanceFatalitiesImprovement = Math.ceil(improvedFatalityRate * Math.pow(zScoreFull / (numberOfFatalities - improvedFatalityRate), 2));
	...
	rates.forEach((r, index) => {
		...
		improvementRates.forEach((improvementPercentage) => {
			...
			const distance = Math.ceil(improvedRate * Math.pow(zScoreFull / (r - improvedRate), 2));
	...
	resultsImprovement.innerHTML = `How many ${unit} would autonomous vehicles have to be driven to demonstrate with ${(confidenceInterval * 100).toString()} % confidence that their failure rate is ${(improvement * 100).toString()} % better than the human driver failure rate of`;
	
	var results = '<ul>';
	results += `<li>${numberOfCrashes} crashes per ${unit}: ${formatNumber(distanceCrashesImprovement)} ${unit}</li>`;
	results += `<li>${numberOfInjuries} injuries per ${unit}: ${formatNumber(distanceInjuriesImprovement)} ${unit}</li>`;
	results += `<li>${numberOfFatalities} fatalities per ${unit}: ${formatNumber(distanceFatalitiesImprovement)} ${unit}</li>`;
	results += '</ul>';
